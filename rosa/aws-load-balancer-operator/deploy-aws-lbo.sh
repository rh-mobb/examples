...
echo "--> Fetch cluster information"
...
echo "--> Create IAM for albo"

echo "Cluster: ${ROSA_CLUSTER_NAME}, Region: ${REGION}, OIDC Endpoint: ${OIDC_ENDPOINT}, AWS Account ID: ${AWS_ACCOUNT_ID}"
SUBSTITUTED_POLICY=$(cat ${SCRIPT_DIR}/trust-policy.json | envsubst)
...
for subnet in $(rosa describe cluster -c $ROSA_CLUSTER_NAME -o json | jq -r '.aws.subnet_ids[]'); do
aws ec2 create-tags \
  --resources "${subnet}" \
